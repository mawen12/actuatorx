...
export const padValue = (value: number) => (value < 10 ? `0${value}` : `${value}`)
...
export const formatCountDown = (millis: number) => {
...
export const formatInterval = (millis: number) => {
    if (millis === 0) {
        return '0'
    }
...
export const formatWidgetChartValue = (value: number, valueType: string) => {
...
export const formatWidgetValue = (value: JsonValue, valueType: string) => {
