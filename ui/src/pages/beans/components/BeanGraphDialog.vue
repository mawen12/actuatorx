...
const edges = computed(() =>
  uniqBy(allBeans.value ?? [], (b) => b.name).flatMap((b) =>
    (b.dependencies ?? [])
...
const nodes = computed(() =>
  uniqBy(allBeans.value ?? [], (b) => b.name).map((b) => ({
    id: b.name.toString(),
    data: {
      label: b.shortName,
      package: b.package,
...
<template>
  ...
  <v-dialog v-model="state.visible" fullscreen transition="none" style="margin: 64px">
    ...
    <v-card class="dialog-card rounded-lg" style="height: 100%; max-height: 100%; width: 100%; max-width: 100%">
      <v-card-title class="d-flex align-center" :elevation="0">
        ...
        <v-tooltip location="bottom">
          <template v-slot:activator="{ props }">
            <v-btn v-bind="props" variant="text" icon="mdi-close" @click.stop="closeBeanGraphDialog()" />
          </template>
          Close
        </v-tooltip>
      </v-card-title>
      ...
      <v-card-text class="page-content pa-0">
        <GraphContext :nodes="connectedNodes" :edges="connectedEdges" :initial-selected-node="initialSelectedNode">
          <GraphComponent />
        </GraphContext>
      </v-card-text>
    </v-card>
